...
async def files_search_tool(stock_name: str) -> str:
    """
...
async def files_search_agent(stock_name: str) -> str:
    """Agent function for 'expert opinions', calls expert_opinions_tool."""
    return await files_search_tool(stock_name)
...
async def files_search_tool(agent_name ,stock_name: str) -> str:
    """
    A dedicated Bing call focusing on the latest news for 'stock_name'.
    """
    print(f"[files_search_tool] files_search_tools for {stock_name}...")
    thread = project_client.agents.create_thread()
    message = project_client.agents.create_message(
        thread_id=thread.id,
        role="user",
    ...
    run = project_client.agents.create_and_process_run(thread_id=thread.id, assistant_id=agent_name.id)
    messages = project_client.agents.list_messages(thread_id=thread.id)
...
async def initialize() -> tuple[Agent, AgentThread]:
    ...
    try:
        ...
        agent = await project_client.agents.create_agent(
            ...
            instructions="Search through attached files",
            toolset=toolset,
...
file_search_agent_assistant = AssistantAgent(
    name="stock_trends_agent",
    model_client=az_model_client,
    tools=[files_search_agent],
    system_message=(
...
async def main():
    ...
    try:
        file_search_agent, thread = await initialize()
